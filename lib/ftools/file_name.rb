...
module FTools
  ...
  def self.new_basename( old_basename, date_time_original: DateTime.civil, author: "N_A" )
    %Q{#{date_time_original.strftime('%Y%m%d-%H%M%S')}_#{author} #{old_basename}}    
  end

  def self.clean_name( basename )
    ...
    if (/^(\d{8}-\d{4}[-_\s]\w{3}[-\s_])(.*)/ =~ basename)
      return $2
    end
    # check if name = YYYYmmdd-HHMM_name
    if (/^(\d{8}-\d{4}[-\s_])(.*)/ =~ basename)
      return $2
    end
